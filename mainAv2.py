...
def creatingTupleMissing2(registre, tuple,missingCount):
    ...
    for k in range(len(registre[0])):
        if registre[tuple[0]][k] == empty or registre[tuple[1]][k] == empty or registre[8][k] or registre[9][k] < missingCount:
            ...
            uplet.append(hashlib.sha256((registre[tuple[0]][k] + str(registre[tuple[1]][k])).encode('utf-8')).hexdigest()) # if the tuple is not empty or already linked, we concatenate its component and hash the concatenation
    return(uplet)
...
def creatingTupleMissing3(registre, tuple,missingCount):
    ...
    for k in range(len(registre[0])):
        if registre[tuple[0]][k] == empty or registre[tuple[1]][k] == empty or registre[tuple[2]][k] == empty or registre[8][k] or registre[9][k] < missingCount:
            ...
            uplet.append(hashlib.sha256((registre[tuple[0]][k] + registre[tuple[1]][k] + str(registre[tuple[2]][k])).encode('utf-8')).hexdigest())  # if the tuple is not empty or already linked, we concatenate its component and hash the concatenation
    return(uplet)
